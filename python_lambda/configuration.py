...
BACKTEST_OUTPUT_PATH = os.getenv(key='LAMBDA_OUTPUT_PATH',default=str(PROJECT_PATH)+os.sep+'output')# must be the same as application.properties
BACKTEST_TEMP_PATH = os.getenv(key='LAMBDA_TEMP_PATH',default=str(PROJECT_PATH)+os.sep+'temp')# must be the same as application.properties
default_jar_path = str(pathlib.Path(PROJECT_PATH).parent)+os.sep+os.path.join('java','executables','Backtest','target','Backtest.jar')
...
if not "initiliazed_configuration" in locals():
    ...
    def create_application_properties():
        ...
        for key in dict_write.keys():
            output+='%s:%s'%(key,dict_write[key].replace(os.sep,os.sep+os.sep))+os.linesep
