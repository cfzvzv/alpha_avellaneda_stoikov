...
class GAParameterTuning:
    ...
    def _generate_initial_population(self, check_unique_population: bool) -> list:
        ...
        for param_dict in param_dicts:
            print('initial [%d]'%(self.counter_algorithms+param_dicts.index(param_dict))+json.dumps(param_dict, indent=4))
    ...
    def _generate_next_gen(self, check_unique_population: bool) -> list:
        ...
        while len(param_dicts) < self.ga_configuration.population:
            ...
            param_dicts.append(param_dict)
        if not self.INCREASE_POPULATION:
            print("generation %d with new %d mutations and %d crossover"%(self.generation,mutation_count,crossover_count))
        else:
        ...
        for param_dict in param_dicts:
            print('[%d]'%(self.counter_algorithms+param_dicts.index(param_dict))+json.dumps(param_dict, indent=4))
        return param_dicts
    ...
    def _run_backtests(
        ...
        for algorithm in algorithms:
            ...
            if algorithm_info not in output_dict:
                score=0.
                pnl=0.
                trades=0
                sharpe=-999
            else:
                returns = output_dict[algorithm_info][
