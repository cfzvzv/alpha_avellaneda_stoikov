...
DEFAULT_PARAMETERS_CURRENT['fastPeriodMin'] = 5
DEFAULT_PARAMETERS_CURRENT['slowPeriodMin'] = 25
DEFAULT_PARAMETERS_CURRENT["changeSide"] = 0


class SMACross(AvellanedaDQN):
    NAME = AlgorithmEnum.sma_cross
    ...
    def train(self, start_date: datetime.datetime, end_date: datetime, instrument_pk: str, iterations: int,
        ...
        self.parameters['fastPeriodMin'] = -1
        self.parameters['slowPeriodMin'] = -1
...
if __name__ == '__main__':
    avellaneda_dqn_directional = SMACross(algorithm_info='test_main_dqn')
    ...
    #
    # )
    avellaneda_dqn_directional.set_parameters(parameters=parameters_base_pt)
    ...
    ga_configuration.population = 4

    best_param_dict, summary_df = avellaneda_dqn_directional.parameter_tuning(
        instrument_pk='btcusdt_binance',
        start_date=datetime.datetime(year=2020, day=7, month=12, hour=7),
        end_date=datetime.datetime(year=2020, day=7, month=12, hour=9),
        parameters_base=parameters_base_pt,
        parameters_min={"fastPeriodMin": 1, "slowPeriodMin": 16},
        parameters_max={"fastPeriodMin": 15, "slowPeriodMin": 35},
        generations=3,
        max_simultaneous=1,
        ga_configuration=ga_configuration,
    )
    print('best parameters as %s' % best_param_dict)
    avellaneda_dqn_directional.set_parameters(parameters=best_param_dict)

    print('Starting testing')
    output_test = avellaneda_dqn_directional.test(
        instrument_pk='btcusdt_binance',
    ...
    avellaneda_dqn_directional.plot_trade_results(
        raw_trade_pnl_df=output_test[avellaneda_dqn_directional.algorithm_info], title='test')
    plt.show()
    avellaneda_dqn_directional.plot_params(raw_trade_pnl_df=output_test[avellaneda_dqn_directional.algorithm_info])
    plt.show()
