...
public class OrderbookManager {
	...
	public OrderbookManager(Orderbook orderbook, PaperExecutionReportConnectorPublisher paperTradingEngineConnector) {
		this.orderbook = orderbook;
		this.paperTradingEngineConnector = paperTradingEngineConnector;

		mmBidLevelToClientOrderId = new ConcurrentHashMap<>();
		mmAskLevelToClientOrderId = new ConcurrentHashMap<>();
		clientOrderIdToOrdId = new ConcurrentHashMap<>();
		ordIdToClientOrderId = new ConcurrentHashMap<>();
		clientOrderIdToOrderOrderbook = new ConcurrentHashMap<>();
		executionReportMap = new ConcurrentHashMap<>();
		asyncNotification = new ConcurrentHashMap<>();
		marketMakerActiveOrders = new ConcurrentHashMap<>();
	...
	private void updateOrderResult(ExecutionReport executionReport, Trade trade) {
		...
		executionReport.setTimestampCreation(orderbook.getTime());//get current orderbook time
	...
	public boolean treatOrderReport(OrderReport orderReport, OrderRequest orderRequest,
		...
		if (isNewOrModifyOrder) {
			...
			executionReport.setTimestampCreation(orderbook.getTime());
			//analyze trades
	...
	private void notifyExecutionReport(ExecutionReport executionReport) {
	...
	private ExecutionReport generateRejection(OrderRequest orderRequest, String reason) {
		ExecutionReport executionReport = executionReportMap
				.getOrDefault(orderRequest.getClientOrderId(), new ExecutionReport(orderRequest));
		executionReport.setTimestampCreation(orderbook.getTime());
	...
	public void notifyDepthAsync(String instrumentPk) {
		Depth lastSnapshotToNotify = asyncNotification.get(instrumentPk);
		if (lastSnapshotToNotify.getBids().length == 0 || lastSnapshotToNotify.getAsks().length == 0) {
